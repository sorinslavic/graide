...
> **Note:** Port 5173 is fixed in `vite.config.ts`. If it's already in use the server will fail immediately — find and kill the process using `lsof -i :5173` before retrying.
```
...
```
Teacher's Laptop (localhost:5173)
    │
